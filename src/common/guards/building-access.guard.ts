...
import {
  ...
  BUILDING_MANAGER_WRITE_ALLOWED_KEY,
...
export class BuildingAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const allowManagerWrite =
      this.reflector.getAllAndOverride<boolean>(BUILDING_MANAGER_WRITE_ALLOWED_KEY, [
        context.getHandler(),
        context.getClass(),
      ]) ?? false;
    ...
    const allowed =
      accessLevel === 'read'
        ? await this.buildingAccessService.canReadBuildingResource(user, buildingId, {
            requiredPermissions,
            allowResident,
            effectivePermissions: request.effectivePermissions,
          })
        : await this.buildingAccessService.canWriteBuildingResource(user, buildingId, {
            requiredPermissions,
            effectivePermissions: request.effectivePermissions,
            allowManagerWrite,
          });
