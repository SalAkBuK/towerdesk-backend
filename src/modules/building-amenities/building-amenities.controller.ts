...
import { BuildingAmenityResponseDto, toBuildingAmenityResponse } from './dto/building-amenity.response.dto';
...
export class BuildingAmenitiesController {
  constructor(private readonly buildingAmenitiesService: BuildingAmenitiesService) {}
  ...
  async list(
    ...
    const amenities = await this.buildingAmenitiesService.list(user, buildingId);
  ...
  async create(
    ...
    const amenity = await this.buildingAmenitiesService.create(user, buildingId, dto);
