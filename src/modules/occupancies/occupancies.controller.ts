...
import { OccupancyResponseDto, toOccupancyResponse } from './dto/occupancy.response.dto';
...
export class OccupanciesController {
  ...
  async list(
    ...
    const occupancies = await this.occupanciesService.list(user, buildingId);
    return occupancies.map(toOccupancyResponse);
  }
